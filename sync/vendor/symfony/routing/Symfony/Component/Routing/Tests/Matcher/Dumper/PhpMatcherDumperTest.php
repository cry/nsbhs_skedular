...
class PhpMatcherDumperTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function getRouteCollections()
    {
        ...
        $redactedprefixCollection = new RouteCollection();
        $redactedprefixCollection->add('static', new Route('/test'));
        $redactedprefixCollection->add('dynamic', new Route('/{var}'));
        $redactedprefixCollection->addPrefix('redactedprefix');
        $route = new Route('/with-condition');
        $route->setCondition('context.getMethod() == "GET"');
        $redactedprefixCollection->add('with-condition', $route);

        return array(
           array($collection, 'url_matcher1.php', array()),
           array($redirectCollection, 'url_matcher2.php', array('base_class' => 'Symfony\Component\Routing\Tests\Fixtures\RedirectableUrlMatcher')),
           array($redactedprefixCollection, 'url_matcher3.php', array()),
        );
    }
}
